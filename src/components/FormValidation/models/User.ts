...
export const BasicUserSchema = z.object({
  ...
  username: z.string().trim().toLowerCase().min(4, { message: 'Username must be 4 or more characters long' }),
...
// `extend` will work as expected in TypeScript (e.g. like an interface extending another interface)
export const UserFormSchemaWithAddress = BasicUserSchema.extend({
  address: UserAddressSchema,
});

// `merge` is like `intersection i.e. '&'` so it will take from both `UserFormSchemaWithAddress` and `HasIDSchema`
